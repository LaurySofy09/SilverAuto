<?php
    ...
    if($mes == ""){
    	...
    	$detalle = $objVenta->Imprimir_Corte_Z_Mes(date('m-Y'));

    } else {

    	$detalle = $objVenta->Imprimir_Corte_Z_Mes($mes);
	...
	if($tipo_comprobante == '1')
	{
	    ...
	    strtoupper($meses[date($mes)-1].' del '.$ano), 0,'C',0 ,1);
		...
		$pdf->SetFont('Arial','',10);
		$pdf->Text(2, $get_Y + 87, '-------------------------------------------------------------');
		$pdf->Text(20,$get_Y + 91,'TOTAL VENTA :');
		$pdf->SetFont('Arial','B',10);
		$pdf->Text(50,$get_Y + 91,$p_total_venta);
	...
	$pdf->Output('I','Corte_Z_'.strtoupper($meses[date($mes)-1].' del '.$ano).'.pdf',true);
	} catch (Exception $e) {
