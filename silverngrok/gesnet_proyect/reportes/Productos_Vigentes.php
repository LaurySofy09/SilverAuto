...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(105);
            $this->Cell(170,10,'REPORTE DE PRODUCTOS VIGENTES',0,0,'C');
            $this->Ln(20);
        }
    }
...
$listado = $objProducto->Listar_Productos_Vigentes();
...
try {
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        $pdf->Text(10,$get_Y + 10,'TOTAL DE PRODUCTOS VIGENTES : '.number_format($total, 2, '.', ','));
    }

    $pdf->Output('I','Productos_Vigentes.pdf');
