...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(98);
            // Title
            $this->Cell(105,10,'COMPRAS (DETALLADAS) AL CREDITO DEL MES '.strtoupper($meses[date((int)$mes)-1].' del '.$ano),0,0,'C');
...
try {
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        $pdf->Text(10,$get_Y + 20,'TOTAL DE ITBMS EN COMPRAS : '.number_format($total_iva, 2, '.', ','));
