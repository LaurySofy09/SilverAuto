$(function() {
  ...
  // Initialize with options
    ...
    jQuery.validator.addMethod("greaterThan",function (value, element) {
     ...
     /*var validator = $("#frmPago").validate({
      ...
      rules: {
        ...
        cbClienteCot:{
          required: function() {
                   return $("#chkPagadoCot").prop('checked') == false;
            }
        },
        txtNoTarjeta:{
          required: function() {
    ...
    var form = $('#frmPago');
     $('#cbClienteCot', form).change(function () {
...
$('#modal_iconified_cash').on('shown.bs.modal', function (e) {
  ...
  $("#txtMonto").focus();
   $("#txtDeuda").val($("#hdnApagar").val()); 
});

$('#modal_iconified_cash').on('hidden.bs.modal', function () {
    $("#cbClienteCot").select2("val", "All");
    $("#txtLimitCot").val("");
  //  $("#txtDeuda").val('');
    $("#txtMonto").val('');
    $("#txtCambio").val('');
...
function mitad_pago(){
  ...
  if($("#chkPagadoCot").prop('checked') == true){
...
var mySwitch = new Switchery($('#chkPagadoCot')[0], {
    size:"small",
    color: '#19AA8D',
...
function Venta_Credito(){
  ...
  var limite_credito = $("#txtLimitCot").val();
        ...
        if(inventariable == 0){
          ...
          agregar_detalle(idproducto,producto,datos,precio_venta,precio_venta1, precio_venta2, precio_venta3, 0.00,stock,perecedero,inventariable);
            ...
            if(exento == 0)
            {
              ...
              if(tipo_precio == 1)
              {
                agregar_detalle(idproducto,producto,datos,precio_venta,precio_venta1, precio_venta2, precio_venta3,precio_venta,stock,perecedero,inventariable);
...
function enviar_data(){
  ...
  var i=0;
  var StringDatos="";
  var pagado = $('#chkPagadoCot').is(':checked') ? 1 : 0;
  var comprobante = $("#cbCompro").val();
  var idcliente = $("#cbClienteCot").val();
    ...
    $("#tbldetalle tbody tr").each(function (index)
        {
            ...
            $(this).children("td").each(function (index2)
            {
                switch (index2)
                {
                    ...
                    case 5:  campo5 = $(this).find("#tbldesc").val();
                             descuento = 0;// parseFloat(campo5);
                             break;

                    case 6:  campo6 = campo2 * campo3
                             importe = campo6; //parseFloat(campo6);
        ...
        StringDatos+=campo0+"|"+cantidad+"|"+precio_unitario+"|"+
        exentos+"|"+descuento+"|"+fecha_vence+"|"+importe+"#";
        i=i+1;
        }
		...
		var idCotizacion = $("#hiddenCotiza").val();
        ...
        if(total > 0.00){
            ...
            success: function(data){
              if(data=="Validado"){
				...
				var dataCotiza = 'proceso=fact&idCotizacion='+idCotizacion;
				$.ajax({
				  type:'POST',
