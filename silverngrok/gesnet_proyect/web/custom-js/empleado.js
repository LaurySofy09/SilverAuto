...
function openEmpleado(action, idempleado, codigo_empleado, nombre_empleado, apellido_empleado, telefono_empleado, email_empleado, estado, imagen)
 {
    ...
    $('#modal_iconified').on('shown.bs.modal', function () {
     ...
     if (action == 'nuevo'){
      ...
      $('#txtEmail').val(email_empleado);
      		if(imagen!= ''){
				$(".peditar").attr("src", "web/images_user/"+imagen);
			}else{
				$(".peditar").attr("src", "web/assets/images/no_img.jpg");
			}
	 $('#cimagen').val(imagen);
...
if($("#txtImagen").val() !=''){
  ...
  dataString+='&telefono_empleado='+telefono_empleado+'&email_empleado='+email_empleado+'&imagen='+imagen+'&cimagen='+cimagen;
