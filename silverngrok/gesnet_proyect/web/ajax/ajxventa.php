<?php
    ...
    if (!empty($_POST))
    {
        try {
          ...
          $numero_tarjeta =  str_replace ( "-", "", $numero_tarjeta);
            ...
            if($idcliente==''){
                if($pagado == '1'){
                        ...
                        $pago_tarjeta,$numero_tarjeta,$tarjeta_habiente,$cambio,1,0,$_SESSION['user_id'], $pagado);
                } else if ($pagado == '0') {
                        $funcion->Insertar_Venta($tipo_pago,$comprobante,$sumas,$iva,$exento,$retenido,$descuento,$total,$son_letras,$efectivo,
                        $pago_tarjeta,$numero_tarjeta,$tarjeta_habiente,$cambio,2,0,$_SESSION['user_id'], $pagado);
                }
            } else if ($idcliente!='') {
                if($pagado == '1'){
                    $funcion->Insertar_Venta($tipo_pago,$comprobante,$sumas,$iva,$exento,$retenido,$descuento,$total,$son_letras,$efectivo,
                    $pago_tarjeta,$numero_tarjeta,$tarjeta_habiente,$cambio,1,$idcliente,$_SESSION['user_id'], $pagado);
                } else if ($pagado == '0') {
                    $funcion->Insertar_Venta($tipo_pago,$comprobante,$sumas,$iva,$exento,$retenido,$descuento,$total,$son_letras,$efectivo,
                    $pago_tarjeta,$numero_tarjeta,$tarjeta_habiente,$cambio,2,$idcliente,$_SESSION['user_id'], $pagado);
                }
            }
         ...
         $data = "Error ajxventa";
	     error_log("Error ajxVenta: " . $e->getMessage());
